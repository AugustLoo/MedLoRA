...
总步数应为 **981**（与 C2-300 相同, 样本数没变）。

**实测 (2026-09-24)**: 训练 45 分 44 秒 + 评估四张表约 40 分钟, `srb` 待机时的速度。结果 macro-F1 59.80 (种子 42 是 60.48),
两条结论都站住了, 见 `results/README.md`。
...
**为什么**: TextVQA 只有 300 题, 还是短答式 OCR 题, 和微调后的输出格式太像, 当时担心会低估遗忘 (实测恰恰相反, 见本节末尾)。
...
### 训练慢 —— 现在有了最像的解释: 别人的任务在抢卡

**2026-09-24 新证据**: C2-300-s43 那轮 981 步只用 45 分 44 秒 (2.8 秒一步), 当时另一账号的 `srb` 在但 0% 待机。
四轮对照: C1 4.7 秒 (`srb` 不在) / A-server 38 秒 (`srb` 在, 负载未记录) / C2-300 40 秒 (`srb` 两卡 100%, 575 W) / s43 2.8 秒 (`srb` 待机)。
快慢与那个任务是否满载对应 —— GPU 分时共享, 另一进程满载时我们的内核排队, 表现为我们的进程闲、整卡看着也不忙。
**这是相关性不是证明** (A-server 那轮的 `srb` 负载没记录; C2-300 那轮采到整卡利用率只有 4–15%, 可能间歇满载)。
**操作纪律: 开跑前看 `nvidia-smi` 里别人进程的利用率。闲着就跑, 一小时出结果; 满载就等。**
